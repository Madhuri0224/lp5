...
int main(int argc, char *argv[])
{
	...
	if (rank == 0)
	{
		...
		for (j = 0; j < SIZE; ++j)
		{
			unsorted_array[j] = (int)rand() % 10000;
		}
		printf("Created\n");
	}
	...
	if (rank == 0)
	{
		...
		if (error)
			...
			printf("Correct! sorted correctly\n");
	}
...
// gcc quicksort_merge_mpi.c - o quicksort_merge_mpi - I "C:\Program Files (x86)\Microsoft SDKs\MPI\Include" - L"C:\Program Files (x86)\Microsoft SDKs\MPI\Lib\x64" - lmsmpi
// ./ quicksort_merge_mpi